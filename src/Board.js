...
export default class Board extends React.Component {
  ...
  setupDragAndDrop() {
    ...
    console.log("Setting up drag and drop with refs:", this.swimlanes);
    ...
    Dragula(containers)
      ...
      .on("drop", (el, target, source) => {
        ...
        const swimlane = el.closest(".Swimlane-dragColumn");
        const newStatus = swimlane
          ? swimlane.getAttribute("data-status")
          : null;
        this.updateCardStatus(el, cardId, newStatus);
      })
  ...
  updateCardStatus(el, cardId, newStatus) {
    ...
    if (newStatus) {
      ...
      this.setState((prevState) => {
        ...
        Object.keys(clients).forEach((key) => {
          clients[key] = prevState.clients[key].map((client) => {
            if (client.id === cardId) {
              return { ...client, status: newStatus.toLowerCase() };
            }
            return client;
          });
        });
